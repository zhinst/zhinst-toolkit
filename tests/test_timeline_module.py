...
@pytest.fixture
def timeline_module(data_dir, mock_connection, session):
    ...
    return TimelineModule(
        mock_connection.return_value.timelineModule(),
        session,
    )


def test_repr(timeline_module):
    assert "MagicMock(DataServerSession(localhost:8004))" in repr(timeline_module)


def test_read(timeline_module, mock_connection, session):
...
def test_subscribe(timeline_module, mock_connection):
    module_mock = mock_connection.return_value.timelineModule.return_value
    timeline_module.subscribe(timeline_module.test.sample.x)
    module_mock.subscribe.assert_called_with("/test/sample.x")


def test_unsubscribe(timeline_module, mock_connection):
    module_mock = mock_connection.return_value.timelineModule.return_value
    timeline_module.unsubscribe(timeline_module.test.sample.x)
    module_mock.unsubscribe.assert_called_with("/test/sample.x")


def test_execute(timeline_module, mock_connection):
    module_mock = mock_connection.return_value.timelineModule.return_value
    timeline_module.execute()
    module_mock.execute.assert_called_with()


def test_finish(timeline_module, mock_connection):
    module_mock = mock_connection.return_value.timelineModule.return_value
    timeline_module.finish()
    module_mock.finish.assert_called_with()


def test_finished(timeline_module, mock_connection):
    module_mock = mock_connection.return_value.timelineModule.return_value
    timeline_module.finished()
    module_mock.finished.assert_called_with()


def test_progress(timeline_module, mock_connection):
    module_mock = mock_connection.return_value.timelineModule.return_value
    timeline_module.progress()
